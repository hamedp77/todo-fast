...
def create_new_todo(new_todo: str) -> None:
    ...
    res = httpx.post(
        f'{API_URL}/todos',
        headers={API_ACCESS_HEADER: user_token, 'Content-Type': 'application/json'},
        json={'todo': new_todo},
    )
    if res.is_success:
...
def login() -> None:
    ...
    with st.form('login_form'):
        ...
        if login_btn:
            ...
            if res.is_success:
                st.session_state['user_token'] = res.json().get(API_ACCESS_HEADER)
...
def render_tasks_section(user_token: str) -> None:
    res = httpx.get(f'{API_URL}/todos', headers={API_ACCESS_HEADER: user_token})
    if res.is_success:
        todos = res.json()
        with st.expander(f'Tasks ({len(todos)})', expanded=True, icon='📝'):
            ...
            filter_ = st.pills('Filters:', options=filters, default='All')
            filtered_todos = []
            match filter_:
                case 'All':
                    filtered_todos = todos
                case 'Done':
                    filtered_todos = [i for i in todos if i.get('done')]
                case 'Not done':
                    filtered_todos = [i for i in todos if not i.get('done')]
            ...
            for todo_item in filtered_todos:
                ...
                with st.container(border=True):
                    ...
                    with col3:
                        st.button(
                            '🖉',
                            key=f'edit-btn-{todo_id}',
                            on_click=edit_todo,
                            args=(todo_id, todo_text),
                        )
    else:
        st.error(res.json().get('detail'), icon='❌')
